/proc/priority_announce(text, title = "", sound = 'sound/ai/attention.ogg', type , sender_override)
	...
	if(type == "Priority")
		...
		if (title && length(title) > 0)
			announcement += "<br><h2 class='alert'>[lhtml_encode(title)]</h2>"
	...
	announcement += "<br><span class='alert'>[lhtml_encode(text)]</span><br>"
	announcement += "<br>"
...
/proc/minor_announce(message, title = "Attention:", alert)
	...
	for(var/mob/M in GLOB.player_list)
		if(!isnewplayer(M) && M.can_hear())
			to_chat(M, "<span class='big bold'><font color = red>[lhtml_encode(title)]</font color><BR>[lhtml_encode(message)]</span><BR>")
			if(M.client.prefs.toggles & SOUND_ANNOUNCEMENTS)
				if(alert)
